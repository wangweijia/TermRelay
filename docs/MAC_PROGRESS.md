...
> 评估日期：2026-09-10
>
> 当前阶段：阶段 0 已完成，可以进入阶段 1
>
> 评估范围：`apps/mac` 现有源码、测试、项目规划及 Git 提交记录

## 总体结论

macOS 端已经完成阶段 0 的本地终端与输出分流实现。App 在没有 Server 的情况下可以作为普通终端运行，并能在指定目录启动登录 Shell 或 Codex。Server WebSocket 闭环、多窗口和产品化能力仍未实现。

- 工程骨架完成度：约 40%。
- Mac MVP 功能完成度：约 25%～30%。
- 当前可实际使用程度：可作为单窗口本地终端原型使用。
- 当前可以完成：选择目录、启动登录 Shell/Codex、PTY 交互、终端渲染、Ctrl-C、停止及输出批处理探针。
- 当前无法完成：连接 Server、远程控制、多窗口、断线补传和正式应用分发。
