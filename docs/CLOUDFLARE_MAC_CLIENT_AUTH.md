...
```text
POST /api/mac-pairings
POST /api/mac-pairings/token
WSS  /ws/mac-client
```
...
- 公网 Mac 使用 `wss://termrelay.wqyhomes.com/ws/mac-client`。
...
sequenceDiagram
    ...
    App->>Public: POST /api/mac-pairings
    ...
    App->>Public: POST /api/mac-pairings/token with deviceCode
...
Mac 调用公开的 `POST /api/mac-pairings`，提交设备 ID、显示名称、App 版本以及可选的本机公钥。Server
创建短期配对记录并返回：
...
```json
{
  ...
  "userCode": "ABCD-EFGH",
  "verificationURL": "https://termrelay.wqyhomes.com/mac/authorize?code=ABCD-EFGH",
  "expiresIn": 600,
  "pollInterval": 3
}
```
...
App 使用 `NSWorkspace.shared.open` 打开 `verificationURL`。`/mac/authorize` 和
`/api/mac-approvals` 不在 Bypass 范围内，浏览器必须先完成现有 Cloudflare Access Email OTP 或 IdP
登录。
...
Mac 按 Server 返回的间隔轮询 `POST /api/mac-pairings/token`，提交 `pairingId` 和 `deviceCode`。
...
创建和过期时间、最后使用时间及撤销时间。服务端持久状态使单设备撤销和轮换保持简单。
...
具体的 Self-hosted Application，例如 `termrelay-mac-public`：

| Path | Policy | 用途 |
| --- | --- | --- |
| `/api/mac-pairings` | Bypass / Everyone | 创建配对 |
| `/api/mac-pairings/token` | Bypass / Everyone | 轮询并兑换凭据 |
| `/ws/mac-client` | Bypass / Everyone | 已认证设备的公网 WebSocket |

注意事项：

- 不要 Bypass `/api/*`、`/ws/*`、`/ws/web` 或 `/ws/client`。
- Cloudflare 的 `path/*` 不匹配父路径 `path`；应逐个配置精确路径，避免依赖宽泛通配符。
- `/mac/authorize` 和 `/api/mac-approvals` 必须继续命中整站 Access Application。
...
- `MacPairingService`：创建、批准、拒绝、轮询和一次性兑换配对。
- `DeviceCredentialService`：生成、哈希、验证、轮换和撤销凭据。
- `CloudflareAccessVerifier`：只用于受保护的浏览器批准接口，动态获取并缓存团队 JWKS。
- 公网 WebSocket Gateway：只服务 `/ws/mac-client`，在 Upgrade 阶段验证 Bearer 凭据。
...
- 公网连接默认使用 `wss://termrelay.wqyhomes.com/ws/mac-client`。
- 局域网 `ws://.../ws/client` 保留无凭据连接能力。
- 收到 `401` 或凭据撤销状态后停止指数重连，提示重新授权；普通网络错误继续退避重连。
...
- WebSocket `401`：清除或标记正式凭据失效，要求重新授权。
- Server 暂时不可达：保留未撤销且未过期的正式凭据，使用现有网络退避重连。
...
1. 未登录浏览器访问 `/mac/authorize` 时进入 Cloudflare Access 登录。
2. 未授权客户端不能连接 `/ws/mac-client`。
3. Mac 创建配对后，用户可在浏览器核对并批准指定设备。
4. 配对凭据只能兑换一次，过期或拒绝后不能兑换。
5. 批准后 Mac 自动连接 `wss://termrelay.wqyhomes.com/ws/mac-client`。
6. 公网下注册、心跳、Session 同步、输入、resize、interrupt 和审批正常。
7. Web 管理页撤销设备后，新连接立即失败；可选地关闭现有连接。
...
3. **公网 Gateway**：新增 `/ws/mac-client` 并在 Upgrade 前完成鉴权。
