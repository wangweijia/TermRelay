  ...
  <main class="mobile-shell">
    ...
    <section v-show="activeTab === 'workspace'" class="mobile-screen mobile-workspace">
      <template v-if="relay.selectedSession">
        ...
        <TerminalView v-if="!relay.loadingHistory && relay.selectedSession.runtimeMode === 'pty'" :key="relay.selectedSession.id" :events="relay.selectedEvents" :interactive="relay.selectedSessionInteractive" :scroll-revision="relay.scrollToLatestRevision" mobile-composer @input="relay.sendTerminalInput" @resize="relay.resizeTerminal" />
        <KeepAlive :max="12">
          <StructuredAgentView v-if="!relay.loadingHistory && relay.selectedSession.runtimeMode === 'acp'" :key="relay.selectedSession.id" :events="relay.selectedEvents" :interactive="relay.selectedSessionInteractive" :scroll-revision="relay.scrollToLatestRevision" :shortcut-enabled="false" :has-older="relay.hasOlderBySession[relay.selectedSession.id] ?? false" :loading-older="relay.loadingOlderHistory" :auto-approve-enabled="relay.selectedSession.autoApproveEnabled" :auto-approve-updating="relay.updatingAutoApprove[relay.selectedSession.id] ?? false" @load-older="relay.loadOlderHistory" @start-turn="relay.startToolTurn" @set-configuration="relay.setToolConfiguration" @interrupt="relay.interruptToolTurn" @resolve-approval="relay.resolveApproval" @resolve-user-input="relay.resolveUserInput" @set-auto-approve="(enabled) => relay.setSessionAutoApprove(relay.selectedSession!.id, enabled)" />
        </KeepAlive>
      </template>
