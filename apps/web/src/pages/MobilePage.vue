  ...
  <main class="mobile-shell">
    ...
    <section v-show="activeTab === 'workspace'" class="mobile-screen mobile-workspace">
      <template v-if="relay.selectedSession">
        ...
        <KeepAlive :max="12">
          <StructuredAgentView v-if="!relay.loadingHistory && relay.selectedSession.runtimeMode === 'acp'" :key="relay.selectedSession.id" :events="relay.selectedEvents" :interactive="relay.selectedSessionInteractive" :scroll-revision="relay.scrollToLatestRevision" :shortcut-enabled="false" :has-older="relay.hasOlderBySession[relay.selectedSession.id] ?? false" :loading-older="relay.loadingOlderHistory" @load-older="relay.loadOlderHistory" @start-turn="relay.startToolTurn" @interrupt="relay.interruptToolTurn" @resolve-approval="relay.resolveApproval" @resolve-user-input="relay.resolveUserInput" />
        </KeepAlive>
      </template>
