...
struct DSHStructuredAdapter: StructuredAgentAdapter {
    let providerID = AgentProviderID.dsh
    let displayName = "DeepSeek DSH"
    let configuredExecutableURL: URL?
    let transportFactory: (@Sendable (URL, URL, [String: String]) -> any ACPTransport)?

    init(
        configuredExecutableURL: URL? = nil,
        transportFactory: (@Sendable (URL, URL, [String: String]) -> any ACPTransport)? = nil
    ) {
    ...
    func detect() async throws -> AgentInstallation {
        guard let executable = configuredExecutableURL ?? ExecutableLocator.find(named: "dsh") else {
            throw AgentError.providerUnavailable("找不到 dsh 可执行程序")
        }
        guard FileManager.default.isExecutableFile(atPath: executable.path) else {
            throw AgentError.providerUnavailable("dsh 文件不存在或不可执行：\(executable.path)")
        }
    ...
    func makeRuntime(
        ...
        let runtimeHome = FileManager.default.temporaryDirectory
            .appendingPathComponent("termrelay-dsh-\(configuration.sessionID.uuidString.lowercased())", isDirectory: true)
        ...
        environment["DSH_HOME"] = runtimeHome.path
    ...
    private func readVersion(_ executable: URL) throws -> String {
        ...
        guard process.terminationStatus == 0, !value.isEmpty else {
            throw AgentError.providerUnavailable(
                value.isEmpty ? "无法读取 DSH 版本" : "无法读取 DSH 版本：\(value)"
            )
        }
        return value
    }
...
actor DSHStructuredRuntime: StructuredAgentRuntime {
    ...
    private let client: DSHACPClient
    ...
    init(
        ...
        descriptor = AgentDescriptor(
            ...
            capabilities: [
                ...
                .fileChanges, .approvals, .plans, .usage,
            ]
        )
    ...
    func start() async throws {
        ...
        guard initialized.object?["protocolVersion"]?.integer == 1 else {
            throw AgentError.protocolFailure("DSH 未协商到 ACP v1")
        }
    ...
    func createSession(_ request: AgentSessionRequest) async throws -> AgentSessionReference {
        guard request.sessionID == sessionID,
            ...
            throw AgentError.correlationMismatch("创建请求不属于当前 DSH runtime")
        }
        ...
        let reference = AgentSessionReference(providerID: .dsh, opaqueID: id)
    ...
    func send(_ action: ToolAction) async throws {
        ...
        case .resolveApproval(let resolution):
            guard let pending = pendingApprovals[resolution.approvalID],
                ...
                throw AgentError.correlationMismatch("审批响应不属于当前 DSH turn")
            }
            ...
            if resolution.decision == .cancel {
                ...
                throw AgentError.protocolFailure("该 DSH 审批不支持 \(resolution.decision.rawValue)")
            }
    ...
    func stop() async {
        ...
        if let providerSessionID {
            ...
            _ = try? await client.request(method: "session/close", params: .object([
                "sessionId": .string(providerSessionID),
            ]), timeout: .seconds(10))
        }
        promptTask?.cancel()
    ...
    private func runPrompt(sessionID: String, turnID: String, text: String) async {
        do {
            ...
            emit(.failed(code: "dsh_prompt", message: error.localizedDescription), turnID: turnID)
    ...
    private func receive(_ message: ACPMessage) async {
        switch message {
    ...
    private func receiveSessionUpdate(_ params: JSONValue) {
        ...
        case "tool_call":
            ...
            let title = update["title"]?.string ?? "DSH 工具"
            let input = update["rawInput"].flatMap(jsonDescription)
            let command = input.map { "\(title)\n\($0)" } ?? title
            emit(.commandStarted(commandID: toolID, command: command), turnID: turnID, itemID: toolID)
    ...
    private func receivePermissionRequest(id: JSONValue, params: JSONValue) async {
        ...
        let approvalID = "dsh:\(toolID):\(rpcKey(id))"
        var decisions = ApprovalDecision.allCases.filter { optionIDs[$0] != nil }
        decisions.append(.cancel)
        let title = tool["title"]?.string ?? "批准 DSH 工具操作"
        let detail = tool["rawInput"].flatMap(jsonDescription)
